...
class Follower:
    def __init__(self):
        ...
        image = rospy.Subscriber('drone06/downward_cam/camera/image', Image, self.image_callback)
        self.data = rospy.Subscriber('/gazebo/model_states', ModelStates, self.callback)
        self.cmd_vel_pub = rospy.Publisher('drone06/cmd_vel',Twist, queue_size=10)
        self.twist = Twist()
    ...
    def callback(self, data):
        ...
        dist_67 = float(math.sqrt((d06_posX-d07_posX)**2 + (d06_posY-d07_posY)**2))

        if d06_posZ < 2 and dist_56 > 15:            
            calc_odom = (Vector3(-0.1*(d06_velX-d05_posX),-0.1*(d06_posY-d05_posY),-0.05*(d06_posZ-3)))
            print("ATR APPEAR!!")

        else:
            if d06_posZ >= 2 and curr_time > 10:

                if dist_56 <= MAX_DIST  and d05_posZ >= 2:
                    #print("d56 REPUL APPEAR!!", dist_56)
                    b56 = -1.8*float((20.0/(MAX_DIST**7))*(dist_56**7) - (70.0/(MAX_DIST**6))*(dist_56**6) + (84.0/(MAX_DIST**5))*(dist_56**5) - (35.0/(MAX_DIST**4))*(dist_56**4) +1)

                else:
                    b56 = 0.0
                    
                if dist_67 <= MAX_DIST  and d07_posZ >= 2:
                    #print("d67 REPUL APPEAR!!", dist_67)
                    b67 = 0.4*float((20.0/(MAX_DIST**7))*(dist_67**7) - (70.0/(MAX_DIST**6))*(dist_67**6) + (84.0/(MAX_DIST**5))*(dist_67**5) - (35.0/(MAX_DIST**4))*(dist_67**4) +1)

                else:
                    b67 = 0.0

                repul = round((b56 + b67), 2)
            ...
            calc_odom = (Vector3(new_velX,velY,-0.8*(d06_posZ-3)))
