...
class Follower:
    def __init__(self):
        ...
        image = rospy.Subscriber('/unity_image/compressed_d12', Image, self.image_callback)
    ...
    def callback(self, data):
        ...
        if key == 'd':
            ...
            calc_odom = (Vector3(-0.3*(d12_posX-posX), -0.3*(d12_posY-posY),-3*(d12_posZ-15)))
            ...
            if len(centroid)>0:
                ...
                calc_odom = (Vector3(-0.2*(d12_posX-(cX+2)), -0.2*(d12_posY-(cY+2)),-3*(d12_posZ-15)))
                pub = self.cmd_vel_pub
                pub.publish(Twist(calc_odom, Vector3(0,0,0)))
        rospy.Rate(1).sleep
