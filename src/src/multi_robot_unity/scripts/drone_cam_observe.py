...
class Follower:
    ...
    def image_callback(self, msg):
        ...
        if M_flood['m00'] > 0 and M_ground['m00'] > 0:
            ...
            self.twist.linear.x = -float(err_y)/300.0 
            self.twist.linear.y = -float(err_x)/100.0
            self.twist.angular.z = -float(math.atan2((cy_flood - cy_ground),(cx_flood - cx_ground)))
            self.cmd_vel_pub01.publish(self.twist)
            #print(math.atan2(err_y,err_x))
