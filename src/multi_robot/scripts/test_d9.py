...
class Follower:
    def __init__(self):
        ...
        image = rospy.Subscriber('drone09/downward_cam/camera/image', Image, self.image_callback)
        self.data = rospy.Subscriber('/gazebo/model_states', ModelStates, self.callback)
        self.cmd_vel_pub = rospy.Publisher('drone09/cmd_vel',Twist, queue_size=10)
        self.twist = Twist()
    ...
    def callback(self, data):
        ...
        if d09_posZ < 2 and dist_19 > 15:
            calc_odom = (Vector3(-0.1*(d09_posX-d01_posX),-0.1*(d09_posY-d01_posY),-0.2*(d09_posZ-3)))
            print("ATR APPEAR!!")
        else:
        
            if d09_posZ >= 2 and curr_time > 10:
      
                if dist_19 <= MAX_DIST and d01_posZ >= 2:
                    #print("d19 REPUL APPEAR!!", dist_19)
                    b19 = 1.8*float((20.0/(MAX_DIST**7))*(dist_19**7) - (70.0/(MAX_DIST**6))*(dist_19**6) + (84.0/(MAX_DIST**5))*(dist_19**5) - (35.0/(MAX_DIST**4))*(dist_19**4) +1)

                else:
                    b19 = 0.0

                if dist_98 <= MAX_DIST and d08_posZ >= 2:
                    #print("d98 REPUL APPEAR!!", dist_98)
                    b98 = -0.4*float((20.0/(MAX_DIST**7))*(dist_98**7) - (70.0/(MAX_DIST**6))*(dist_98**6) + (84.0/(MAX_DIST**5))*(dist_98**5) - (35.0/(MAX_DIST**4))*(dist_98**4) +1)

                else:
                    b98 = 0.0         

                repul = round((b19 + b98), 2)
                
            else:
                repul = 0.0

            new_velX = velX - repul
            calc_odom = (Vector3(new_velX,velY,-0.8*(d09_posZ-3)))
